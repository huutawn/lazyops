...
func RegisterRoutes(router *gin.Engine, app *bootstrap.Application) {
	...
	v1 := router.Group("/api/v1")
	{
		...
		userProtected.Use(middleware.RequireAuthKinds(service.AuthKindWebSession, service.AuthKindCLIPAT))
		{
			...
			userProtected.GET("/github/app/config", githubController.AppConfig)
			...
			userProtected.POST("/instances/:id/bootstrap-token", instanceController.IssueBootstrapToken)
			userProtected.POST("/instances/:id/install-agent/ssh", instanceController.InstallAgentViaSSH)
