...
func (ctl *AuthController) Register(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrEmailExists):
			response.Error(c, http.StatusConflict, "register failed", err.Error())
		case errors.Is(err, service.ErrInvalidInput):
			response.Error(c, http.StatusBadRequest, "register failed", err.Error())
		default:
			response.Error(c, http.StatusInternalServerError, "register failed", err.Error())
		}
		return
	}
...
func (ctl *AuthController) Login(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrInvalidInput):
			response.Error(c, http.StatusBadRequest, "login failed", err.Error())
		default:
