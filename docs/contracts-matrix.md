...
| `POST /api/v1/projects/:id/deployments` | backend, frontend | `implemented` | master plan, backend guide | `backend/internal/api/v1/routes.go`, `backend/internal/api/v1/controller/deployment_controller.go` | Tạo bản ghi revision và deployment; tự nó không hoàn tất rollout runtime. |
| `GET /api/v1/projects/:id/topology` | backend, frontend | `implemented` | master plan, backend guide | `backend/internal/api/v1/routes.go`, `backend/internal/api/v1/controller/observability_controller.go` | Mô hình đọc biểu đồ topology. |
| `GET /api/v1/traces/:correlation_id` | backend, CLI, frontend | `implemented` | master plan, backend guide | `backend/internal/api/v1/routes.go`, `backend/internal/api/v1/controller/observability_controller.go`, `cli/internal/command/traces.go` | Đường dẫn đọc tóm tắt trace. |
...
| `GET /ws/operators/stream` | backend, frontend | `implemented` | master plan, backend guide | `backend/internal/api/v1/routes.go` | Luồng operator công khai chính thức. |
...
| `GET /ws/logs/stream` | backend, CLI, frontend | `missing` | master plan, CLI guide | `cli/internal/command/logs.go` | Bộ giải mã xem trước CLI tồn tại, nhưng route backend chưa tồn tại. |
...
| `lazyops doctor` | `/mock/v1/doctor` | `mock-only` | `cli/internal/command/doctor.go` | Route xác thực backend tồn tại cho `lazyops.yaml`, nhưng tóm tắt doctor vẫn còn dựa trên mock. |
| `lazyops status` | danh sách dự án cộng với danh sách binding cộng với khám phá target | `adapter/composed` | `cli/internal/command/status.go` | Không có route `/api/v1/status` chuyên dụng; CLI kết hợp một tóm tắt mỏng. |
| `lazyops logs <service>` | `GET /ws/logs/stream` | `missing` | `cli/internal/command/logs.go` | Bộ phân tích xem trước CLI tồn tại, nhưng luồng backend thực chưa được kết nối. |
