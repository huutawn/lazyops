...
export function PageHeader({ title, subtitle, breadcrumb, actions, className }: PageHeaderProps) {
  return (
    <header className={cn('relative flex flex-col gap-5 pb-6', className)}>
      ...
      {breadcrumb && <nav className="text-sm font-medium text-lazyops-muted">{breadcrumb}</nav>}
      <div className="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex max-w-4xl flex-col gap-2">
          <h1 className="text-3xl font-black tracking-tight text-lazyops-text sm:text-4xl">{title}</h1>
