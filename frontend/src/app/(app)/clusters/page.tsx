  ...
  return (
    ...
    >
      <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <div className="grid gap-3 rounded-2xl border border-[#1e293b] bg-[#020617]/60 p-4">
          ...
          {!managedCluster ? (
            <div className="rounded-xl border border-[#334155] bg-[#0B1120]/70 px-4 py-3 text-sm text-[#94a3b8]">
      ...
      <div className="mt-5 rounded-2xl border border-[#1e293b] bg-[#020617]/60">
        {nodes.isLoading ? (
          <div className="px-4 py-5 text-sm text-[#94a3b8]">Loading nodes...</div>
        ) : nodes.isError ? (
          <div className="px-4 py-5 text-sm text-[#fca5a5]">
            {nodes.error instanceof Error ? nodes.error.message : 'Failed to load cluster nodes.'}
          </div>
        ) : nodeItems.length === 0 ? (
          <div className="px-4 py-5 text-sm text-[#94a3b8]">
            {managedCluster
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#1e293b] text-left text-[#64748b]">
                  ...
                  <th className="px-4 py-3 font-medium">K8s node</th>
                  <th className="px-4 py-3 font-medium">State</th>
                  <th className="px-4 py-3 font-medium">Placement label</th>
    ...
    <tr className="border-b border-[#1e293b]/60 last:border-b-0">
      <td className="px-4 py-3">
        <div className="flex flex-col">
          ...
          <span className="text-xs text-[#94a3b8]">{node.cluster_id}</span>
        </div>
      </td>
      <td className="px-4 py-3 font-mono text-xs text-[#cbd5e1]">{node.instance_id}</td>
      <td className="px-4 py-3 text-[#cbd5e1]">{node.k8s_node_name || 'Waiting for registration'}</td>
      <td className="px-4 py-3">
        <StatusBadge
          label={nodeState}
      ...
      <td className="px-4 py-3 font-mono text-xs text-[#94a3b8]">{placementLabel}</td>
      ...
      {!cluster ? null : (
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-5" noValidate>
          ...
          <FormField label="SSH private key">
            <textarea
              rows={7}
              className="w-full rounded-xl border border-[#1e293b] bg-[#0B1120]/40 px-4 py-3 text-sm text-white outline-none transition-colors placeholder:text-[#64748b]/60 focus:border-[#0EA5E9]/50 focus:ring-4 focus:ring-[#0EA5E9]/10"
              placeholder="-----BEGIN OPENSSH PRIVATE KEY-----"
              {...register('ssh_private_key')}
            />
          </FormField>

          <details className="rounded-2xl border border-[#1e293b] bg-[#0B1120]/50 p-4">
            <summary className="cursor-pointer text-sm font-semibold text-white">Advanced join options</summary>
          ...
          {joinSummary.length > 0 ? (
            <div className="rounded-2xl border border-[#1e293b] bg-[#0B1120]/60 p-4">
              <div className="mb-3 text-sm font-semibold text-white">Recent join stages</div>
              <div className="grid gap-2">
                {joinSummary.map((stage) => (
                  <div key={`${stage.key}-${stage.label}`} className="flex items-center justify-between rounded-xl border border-[#1e293b] bg-[#020617]/70 px-3 py-2 text-sm">
                    <span className="text-[#e2e8f0]">{stage.label}</span>
                    <span className="font-mono text-xs text-[#38bdf8]">{stage.status}</span>
                  </div>
                ))}
              </div>
            </div>
