...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const stateNonce = request.cookies.get('lazyops_oauth_github_state')?.value;
    ...
    if (!response.ok) {
      ...
      const errorCode = errorBody?.error?.code ?? errorBody?.code ?? errorBody?.message ?? 'oauth_failed';
